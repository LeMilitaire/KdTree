...
public class KdTree<Point extends PointI> {
	...
	KdTree(int dim, ArrayList<Point> points, int max_depth) {
		...
		root_= BuildBalancedTree(0,max_depth,(List<Point>) points.clone());
	}
		// creation de l arbre avec methode mediane
		private KdNode BuildBalancedTree(int depth, int max_depth, List<Point> points){
			List<Point> left_points = new ArrayList<Point>();
			List<Point> right_points = new ArrayList<Point>();

			if(points.size() == 0){
				return null;
			}
            // Calcul of the dimension of the cut
            int d= depth%3;

			if(depth == max_depth){
                PointI pt = points.get(0).zero();
				// create a Leaf containing barycenter of last points
                for(int i=0; i<points.size();i++){
                    pt.add(points.get(i));
                }
                pt.div(points.size());
                return new KdNode((Point)pt,d);
			}

            Comparator<Point> compare = new Comparator<Point>() {
                @Override
                public int compare(Point o1, Point o2) {
                    return (o1.get(d)-o2.get(d));
                }
            };

			//sort of points function of the dimension
			points.sort(compare);
            Point mediane = points.get(points.size()/2);
			left_points = points.subList(0,points.size()/2);
            right_points = points.subList(points.size()/2+1, points.size());
			//Creation of 2 sorted trees(left and right)
            KdNode left_child = BuildBalancedTree(depth+1,max_depth,left_points);

            KdNode right_child = BuildBalancedTree(depth+1,max_depth,right_points);
            return new KdNode(mediane,d,left_child,right_child);
	}
		// tri des points
		/////////////////
		/// Accessors ///
